...
Matrix *matrixTranspose(const Matrix *matrix)
{
    ...
    for (size_t i = 0u; i < matrix->num_rows; i++)
    {
        ...
        for (size_t j = 0u; j < matrix->num_cols; j++)
        {
            const size_t idx_o = matrixIndex(matrix->num_cols, i, j);
            const size_t idx_r = matrixIndex(result->num_cols, j, i);

            result->data[idx_r] = matrix->data[idx_o];
        }
    }
...
{
    if (matrix == NULL || matrix->data == NULL || vector == NULL || vector->data == NULL)
        return NULL;
...
Vector *multiplyMatrixByVector(const Matrix *matrix, const Vector *vector)
{
    ...
    for (size_t i = 0u; i < matrix->num_rows; i++)
    {
        for (size_t j = 0u; j < matrix->num_cols; j++)
        {
            ...
            result->data[i] += (matrix->data[idx] * vector->data[j]);
        }
    }
